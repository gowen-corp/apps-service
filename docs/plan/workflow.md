...
1. Закрыть Issue на GitHub
2. Перетащить в Project → Done
3. Обновить статус в `docs/plan/README.md` — ✅
4. Заполнить раздел «Результат» в `N-название.md`
...
## Комментарии к задачам на GitHub

К любой задаче можно добавить комментарий прямо из работы:

```bash
gh issue comment 25 --body "Аудит завершён, отчёт в docs/plan/11-caddy-integration-audit/"
```

Или через AI: «добавь комментарий к #25: сделано, см. коммит abc123»

Это не формализуется в workflow — просто инструмент когда нужно оставить след на GitHub.
